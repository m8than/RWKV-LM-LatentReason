...
PROJ_DIR="out/L"$N_LAYER"-D"$N_EMBD"-"$MODEL_TYPE"-FirstHalfDoubleLayer" # set output folder
#
#######################################################################################################################
#
...
python train.py --load_model "" --wandb "RWKV-7-1B5-CTX" --proj_dir $PROJ_DIR --my_testing $MODEL_TYPE \
 --ctx_len $CTX_LEN --my_pile_stage 1 --epoch_count 999999 --epoch_begin 0 \
